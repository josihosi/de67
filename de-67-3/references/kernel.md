...
The canonical normal-method baseline is the complete active DE-67 method checkout's `de-67-3` tree. Guideline
candidates begin from its `assets/environment/` files and promotion returns only to that tree.
Workspace-local guideline copies are neither baselines nor promotion targets. This prevents an
older project snapshot from replacing unrelated current method behavior while carrying one useful
change.
