...
The active normal-method baseline is the workspace-local mutable guidance under `.de67/`.
Candidates begin from those exact files and successful local promotion returns to those files.
Packaged assets are bootstrap templates, not runtime policy. A separate owner-authorized promotion
may generalize a useful local rule into `de67-lab`; delivery does not depend on that publication.
